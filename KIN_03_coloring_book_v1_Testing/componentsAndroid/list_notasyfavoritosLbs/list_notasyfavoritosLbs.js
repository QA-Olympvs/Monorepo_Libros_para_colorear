class List_notasyfavoritosLbs extends componentBase { 
	...
	async getData() {
        ...
        xmlhttp.open("GET","componentsAndroid/list_notasyfavoritosLbs/list_notasyfavoritosLbs.html", true);
        xmlhttp.send();
    }
	...
	updateStyle(elem) {
		...
		linkCompontCss.setAttribute("href", "componentsAndroid/list_notasyfavoritosLbs/list_notasyfavoritosLbs.css");
	...
	_irANotaHandler(evt) {
		...
		window.parent.IDRViewer.goToPage(pagina);
		...
		setTimeout(() => {
			const notav2Comp = window.parent.document.querySelector('nota-v2-lbs');
	...
	/*
		setTimeout(() => {
			const notaElement = document.getElementById('iframe'+pagina).contentWindow.document.getElementById(idNota);
			...
			document.getElementById('iframe'+pagina).contentWindow.document.getElementById(idNota).scrollTo({ top: scrollPosition,  behavior: 'smooth' });

			setTimeout(() => {

				document.getElementById('iframe'+pagina).contentWindow.document.getElementById(idNota).classList.add('focused');	
				var eventTouchStart = new Event('touchstart');
				
				document.getElementById('iframe'+pagina).contentWindow.document.getElementById(idNota).dispatchEvent(eventTouchStart);
