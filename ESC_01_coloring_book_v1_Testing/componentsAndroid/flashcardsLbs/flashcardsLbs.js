class flashcardsLbs extends componentBase {
    ...
    async getData() {
        ...
        xmlhttp.open('GET', 'componentsAndroid/flashcardsLbs/flashcardsLbs.html', true);
        xmlhttp.send();
    }
    ...
    async _loadCards() {
        ...
        if (inlineCsv) {
            ...
            try {
                raw = await this._loadTextFile(srcPath);
            } catch (err) {
                this._setLoadingMsg('No se pudo cargar el archivo: ' + err.message, true);
                return;
            }
        } else {
