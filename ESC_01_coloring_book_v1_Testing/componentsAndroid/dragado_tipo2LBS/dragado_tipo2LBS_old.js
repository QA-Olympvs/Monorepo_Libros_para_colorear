...
class dragado_tipo2LBS extends componentBase {
    ...
    async getData() {
        ...
        xmlhttp.onreadystatechange = () => {
          if(xmlhttp.status == 200 && xmlhttp.readyState == 4){
			...
			this._drag = this;
            this.objs = this.shadowRoot.querySelector("slot[name='imagen']");
            var cir = this.shadowRoot.querySelector("#circulo");
            var txto = this.shadowRoot.querySelector("#texto");
			//this.strokeColorOriginal = circulo.style.stroke;
			this.tipoEjercicio = this.getAttribute('tipo');	

			if(this.tipoEjercicio === 'e1'){
				
				cir.style.display = '';
				var svg = cir.parentNode;
				svg.setAttribute('display', '');
				this.strokeColorOriginal = cir.style.stroke;
				this._dragTipo1(cir);
				
			} else if(this.tipoEjercicio === 'e2'){
				
				var svg = txto.parentNode;
				txto.style.display = '';
				svg.setAttribute('display', '');
				this.strokeColorOriginal = txto.style.stroke;
				this._dragTipo1(txto);

			}else if(this.tipoEjercicio === 'e3') {
        ...
        xmlhttp.open("GET","componentsAndroid/dragado_tipo2LBS/dragado_tipo2LBS.html",true);
        xmlhttp.send();
	...
	_dragTipo1(dropZone) {
		...
		this.dragImages = this.objs.assignedElements({ flatten: true });
		var overlapThreshold = "80%"; 
        var dragObj = new Draggable.create(this.dragImages);
		const _thisComponent = this;
		console.log({_thisComponent});
		dragObj.forEach(item => {
			...
			item.addEventListener('dragend', (evt) => {
				...
				if(!item.hitTest(dropZone, overlapThreshold)) {
					console.log("test");
					gsap.to(item.target, {
						top: initialY,
						left: initialX,
			...
			item.addEventListener('drag', function(evt) {
				if(!item.hitTest(dropZone, overlapThreshold)) {
					dropZone.style.stroke = _thisComponent.strokeColorOriginal;
				} else {
					dropZone.style.stroke = _thisComponent.strokeColorDentro;
				}
			});
