class mindMapLbs extends componentBase {
    ...
    async getData() {
        ...
        xmlhttp.open("GET", "componentsAndroid/mindMapLbs/mindMapLbs.html", true);
        xmlhttp.send();
    }
    ...
    _ensureGlobalStyles() {
        this._ensureGlobalStylesInDoc(document);
        try {
            if (parent && parent.document && parent.document !== document) {
                this._ensureGlobalStylesInDoc(parent.document);
            }
        } catch (e) {
            // In restricted contexts parent.document may be unavailable.
            console.warn("Could not access parent document to inject styles for mind map component. Some styles may not apply.", e);
        }
    }

    _ensureGlobalStylesInDoc(doc) {
        try {
            if (!doc || !doc.head || doc.getElementById("mm-global-css")) return;
            var link = doc.createElement("link");
            link.id = "mm-global-css";
            link.rel = "stylesheet";
            link.href = "componentsAndroid/mindMapLbs/mindMapLbs.css";
            doc.head.appendChild(link);
        } catch (e) {
            console.error("Error injecting global styles for mind map component.", e);
        }
    }
