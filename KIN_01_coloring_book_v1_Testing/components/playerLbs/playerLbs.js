class playerLbs extends componentBase {
  ...
  _setupAudio() {
    ...
    this.audio = new Audio(`assets/podcast/${audioName}.mp3`);
  ...
  _setupListeners() {
    ...
    shadow.querySelector(".forward").addEventListener("click", () => {
      if (this.audio) {
        // No usar Math.min(duration, ...) — duration puede ser NaN antes de loadedmetadata
        // el browser clampea solo si se pasa del final
