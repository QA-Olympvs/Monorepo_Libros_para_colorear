...
class verNotaLbs extends componentBase {
	...
	async getData() {
        ...
        xmlhttp.open("GET","componentsAndroid/verNotaLbs/verNotaLbs.html",true);
        xmlhttp.send();
	}
	...
	_showComponent(noteData) {
		app.mostrarFabsAudioPlayerCerrar(true);
		...
		this._documentHammer = new Hammer(this.shadowRoot.querySelector("#cerrar-nota"));
	...
	_hideComponent = (event) => {
		const notaImg = document.getElementById('iframe'+this._paginaActial).contentWindow.document.getElementById(this._notaActual);
		notaImg.classList.remove('focused');
			...
			setTimeout(() => {
				...
				app.mostrarFabsAudioPlayerCerrar(false);
			}, 400);
	...
	_handlerCambiarNota(direccion) {
		...
		const notaComp = document.getElementById('iframe'+this._paginaActial).contentWindow.document.getElementById(this._notaActual);
		notaComp.classList.remove('focused');
		...
		setTimeout(() => {
			...
			document.getElementById('iframe'+this._paginaActial).contentWindow.document.getElementById(this._notaActual).scrollIntoView({behavior: 'smooth', block: 'center'});
			document.getElementById('iframe'+this._paginaActial).contentWindow.document.getElementById(this._notaActual).classList.add('focused');
	...
	_deleteDataFirestore() {
		...
		.then(()=> {
			...
			setTimeout(() => {
				this.classList.remove('fadeOutDown');
				this.style.display = 'none';
				document.getElementById('iframe'+this._paginaActial).contentWindow.document.getElementById(this._notaActual).remove();
				app.mostrarFabsAudioPlayerCerrar(false);
				this._txtNotaEdit.value = '';
				this._txtNotaEdit.style.display = 'none';
				this._textoNota.style.display = 'unset';
	...
	_guardarEditNota() {
		const notaEl = document.getElementById('iframe'+this._paginaActial).contentWindow.document.getElementById(this._notaActual);
		console.log("nota actual: ", this._notaActual);

		console.log("Texto: ", this._txtNotaEdit.value);

		if(this._txtNotaEdit.value !== '' && this._txtNotaEdit !== 'none') {
			this._id = this._notaActual;
			this._itemLbs.value = this._txtNotaEdit.value;

			this._extraFields.x = notaEl.getAttribute('x');		
			this._extraFields.y = notaEl.getAttribute('y');
			this._id = notaEl.getAttribute("id");
      		this._pagina = notaEl.getAttribute("pagina");
	...
	_abrirListaNotas() {
		...
		setTimeout(() => {
			...
			window.parent.document.getElementById("list-notas-favoritos").style.display = 'unset';
			window.parent.document.getElementById("list-notas-favoritos")._openListFromVerNotas();
			const notaImg = document.getElementById('iframe'+this._paginaActial).contentWindow.document.getElementById(this._notaActual);
			notaImg.classList.remove('focused');
		}, 400);
