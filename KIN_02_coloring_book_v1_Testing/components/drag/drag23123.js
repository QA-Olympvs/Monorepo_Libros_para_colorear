    ...
    async getData() {
        ...
        xmlhttp.onreadystatechange = () => {
          if(xmlhttp.status == 200 && xmlhttp.readyState == 4){
            ...
            // this._drag = this.shadowRoot.querySelector(".parent");
			this._drag = this;
			...
			dragObj.forEach(item => {
				...
				console.log(_thisComponent.dataInDB);
				item.type = "left,top";	
				if(_thisComponent.dataInDB.length > 0) {
					...
					item.target.style.top = `${_thisComponent.dataInDB[0].y}px`;
					item.target.style.left = `${_thisComponent.dataInDB[0].x}px`;
					setTimeout(() => {
						item.update();
					}, 50);
				}
				...
				item.addEventListener('dragend', (evt) => {
					...
					if(!item.hitTest(circulo, overlapThreshold)) {
						...
						// _thisComponent._setDataExtraFiels('x', item.target.getAttribute('x'));
						// _thisComponent._setDataExtraFiels('y', item.target.getAttribute('y'));
						_thisComponent._setDataExtraFiels('x', item.pointerX);
						_thisComponent._setDataExtraFiels('y', item.pointerY);
						_thisComponent._saveDrag();
					}

					console.log(item.tween);	
				});
